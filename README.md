...
`custom_components/integration_irtrans/translations/*` | [Translation files.](https://developers.home-assistant.io/docs/internationalization/custom_integration)
`custom_components/integration_irtrans/__init__.py` | The component file for the integration.
`custom_components/integration_irtrans/api.py` | This is a sample API client.
`custom_components/integration_irtrans/binary_sensor.py` | Binary sensor platform for the integration.
`custom_components/integration_irtrans/config_flow.py` | Config flow file, this adds the UI configuration possibilities.
`custom_components/integration_irtrans/const.py` | A file to hold shared variables/constants for the entire integration.
`custom_components/integration_irtrans/manifest.json` | A [manifest file](https://developers.home-assistant.io/docs/en/creating_integration_manifest.html) for Home Assistant.
`custom_components/integration_irtrans/sensor.py` | Sensor platform for the integration.
`custom_components/integration_irtrans/switch.py` | Switch sensor platform for the integration.
...
`tests/test_api.py` | Tests for `custom_components/integration_irtrans/api.py`.
`tests/test_config_flow.py` | Tests for `custom_components/integration_irtrans/config_flow.py`.
`tests/test_init.py` | Tests for `custom_components/integration_irtrans/__init__.py`.
`tests/test_switch.py` | Tests for `custom_components/integration_irtrans/switch.py`.
...
3. In the `custom_components` directory (folder) create a new folder called `integration_irtrans`.
4. Download _all_ the files from the `custom_components/integration_irtrans/` directory (folder) in this repository.
